...
public class PP_Service {
    ...
    public static class ServiceRunner implements Runnable {
        ...
        public ServiceRunner(List<URL> URL_pack, List<Proxy> proxies, Map<String, WillRespond> willresponds, Map<String, SimulationWillRespond> simuwillresponds) {
    ...
    private static Map<String, WillRespond> callProxyProvider(List<URL> URL_pack, List<Proxy> proxies, Map<String, WillRespond> willresponds, Map<String, SimulationWillRespond> simuwillresponds) {
            ...
            if (matchingProxy != null) {
                ...
                if (Math.random() > foundSimulation.getProbabilityRejection()){
                    if (foundWillRespond!=null){
                        System.out.println("Called proxy provider service for URL : " + url.getId_URL() + " using Proxy : " + matchingProxy.getId_Proxy() + " successfully.");
                        foundWillRespond.addNewSuccess();
                        foundWillRespond.addNewTry();
                        willresponds.put(key, foundWillRespond);
                    } else {
                        WillRespond newWillRespond = new WillRespond(url.getId_URL(), matchingProxy.getId_Proxy(), 1, 1);
                    ...
                    System.out.println("Proxy provider service hasn't scrapped URL : " + url.getId_URL() + " using Proxy : " + matchingProxy.getId_Proxy());
                    if (foundWillRespond!=null){
                        foundWillRespond.addNewTry();
                        willresponds.put(key, foundWillRespond);
                    } else {
    ...
    //         // String sql = "UPDATE INTO willRespond (Id_URL, Id_Proxy, triedOk, triedTotal) VALUES (?, ?, ?, COALESCE((SELECT triedTotal FROM willRespond WHERE Id_URL = ? AND Id_Proxy = ?), 0) + 1)";
